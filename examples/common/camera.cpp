...
struct Camera
{
    ...
    void setSpeed(float _move, float _mouse)
    {
        m_moveSpeed = _move;
        m_mouseSpeed = _mouse;
    }
...
{
    return s_camera->getHorizontalAngle();
}

float cameraGetVerticalAngle()
{
    return s_camera->getVerticalAngle();
}
...
{
    bx::memCopy(s_camera->m_at, _at, 3 * sizeof(float));
}

void cameraGetUp(float* _up)
{
    bx::memCopy(_up, s_camera->m_up, 3 * sizeof(float));
}

void cameraSetUp(float* _up)
{
    bx::memCopy(s_camera->m_up, _up, 3 * sizeof(float));
}
...
{
    s_camera->setSpeed(_move, _mouse);
}
