...
static void GL_APIENTRY stubDrawArraysInstanced(GLenum _mode, GLint _first, GLsizei _count,
                                                GLsizei /*_primcount*/)
{
    GL_CHECK(glDrawArrays(_mode, _first, _count) );
}

static void GL_APIENTRY stubDrawElementsInstanced(GLenum _mode, GLsizei _count, GLenum _type,
                                                  const GLvoid* _indices, GLsizei /*_primcount*/)
{
...
static void GL_APIENTRY stubObjectLabel(GLenum /*_identifier*/, GLuint /*_name*/,
                                        GLsizei /*_length*/, const char* /*_label*/)
{
}

static void GL_APIENTRY stubInvalidateFramebuffer(GLenum /*_target*/, GLsizei /*_numAttachments*/,
                                                  const GLenum* /*_attachments*/)
{
}

static void GL_APIENTRY stubMultiDrawArraysIndirect(GLenum _mode, const void* _indirect,
        GLsizei _drawcount, GLsizei _stride)
{
...
static void GL_APIENTRY stubMultiDrawElementsIndirect(GLenum _mode, GLenum _type,
        const void* _indirect, GLsizei _drawcount, GLsizei _stride)
{
    const uint8_t* args = (const uint8_t*)_indirect;
...
void GL_APIENTRY debugProcCb(GLenum _source, GLenum _type, GLuint _id, GLenum _severity,
...
void setTextureFormat(TextureFormat::Enum _format, GLenum _internalFmt, GLenum _fmt,
...
static bool isTextureFormatValid(TextureFormat::Enum _format, bool _srgb = false,
                                 bool _mipAutogen = false)
{
    ...
    BX_WARN(0 == err, "TextureFormat::%s is not supported (%x: %s).", getName(_format), err,
            glEnumName(err) );
        ...
        if (BX_ENABLED(BGFX_CONFIG_RENDERER_OPENGLES) )
        {
            ...
            if (BX_ENABLED(BGFX_CONFIG_RENDERER_OPENGLES < 30) )
            {
                ...
                if (BX_ENABLED(BX_PLATFORM_IOS) || BX_ENABLED(BX_PLATFORM_EMSCRIPTEN))
                {
                    ...
                    setTextureFormat(TextureFormat::D24S8, GL_DEPTH_STENCIL,   GL_DEPTH_STENCIL,
...
//          else
        ...
        m_timerQuerySupport &= true
                               ...
                               ;
#if defined(BGFX_DISABLE_TIMER_QUERY_SUPPORT)
        m_timerQuerySupport = false;
#endif
        ...
        if (BX_ENABLED(BGFX_CONFIG_RENDERER_OPENGLES) )
        {
            ...
            s_textureAddress[BGFX_TEXTURE_U_BORDER >> BGFX_TEXTURE_U_SHIFT] =
            ...
            ||  !s_extension[Extension::EXT_debug_marker].m_supported)
        {
            glInsertEventMarker = (NULL != glStringMarkerGREMEDY &&
                                   s_extension[Extension::GREMEDY_string_marker].m_supported)
    ...
    void createVertexBuffer(VertexBufferHandle _handle, Memory* _mem, VertexDeclHandle _declHandle,
                            uint16_t _flags) BX_OVERRIDE
    {
    ...
    void createDynamicIndexBuffer(IndexBufferHandle _handle, uint32_t _size,
                                  uint16_t _flags) BX_OVERRIDE
    {
    ...
    void updateTexture(TextureHandle _handle, uint8_t _side, uint8_t _mip, const Rect& _rect,
                       uint16_t _z, uint16_t _depth, uint16_t _pitch, const Memory* _mem) BX_OVERRIDE
    {
    ...
    void createFrameBuffer(FrameBufferHandle _handle, uint8_t _num,
                           const Attachment* _attachment) BX_OVERRIDE
    {
        m_frameBuffers[_handle.idx].create(_num, _attachment);
    }

    void createFrameBuffer(FrameBufferHandle _handle, void* _nwh, uint32_t _width, uint32_t _height,
                           TextureFormat::Enum _depthFormat) BX_OVERRIDE
    {
    ...
    void submit(Frame* _render, ClearQuad& _clearQuad,
                TextVideoMemBlitter& _textVideoMemBlitter) BX_OVERRIDE;
    ...
    void blitRender(TextVideoMemBlitter& _blitter, uint32_t _numIndices) BX_OVERRIDE
    {
        ...
        if (0 < numVertices)
        {
            ...
            m_vertexBuffers[_blitter.m_vb->handle.idx].update(0, numVertices * _blitter.m_decl.m_stride,
                                                              _blitter.m_vb->data);
    ...
    void setShaderUniform4x4f(uint8_t /*_flags*/, uint32_t _regIndex, const void* _val,
                              uint32_t _numRegs)
    {
    ...
    uint32_t setFrameBuffer(FrameBufferHandle _fbh, uint32_t _height,
                            uint16_t _discard = BGFX_CLEAR_NONE, bool _msaa = true)
    {
        if (isValid(m_fbh)
            &&  m_fbh.idx != _fbh.idx)
        {
    ...
    void createMsaaFbo(uint32_t _width, uint32_t _height, uint32_t _msaa)
    {
            ...
            &&  1 < _msaa)
        {
            ...
            GL_CHECK(glRenderbufferStorageMultisample(GL_RENDERBUFFER, _msaa, GL_DEPTH24_STENCIL8, _width,
                                                      _height) );
            GL_CHECK(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER,
                                               m_msaaBackBufferRbos[0]) );

            GLenum attachment = BX_ENABLED(BGFX_CONFIG_RENDERER_OPENGL) ||
                                BX_ENABLED(BGFX_CONFIG_RENDERER_OPENGLES >= 30)
                                ? GL_DEPTH_STENCIL_ATTACHMENT
                                : GL_DEPTH_ATTACHMENT
                                ;
            GL_CHECK(glFramebufferRenderbuffer(GL_FRAMEBUFFER, attachment, GL_RENDERBUFFER,
    ...
    void blitMsaaFbo()
    {
            ...
            &&  0 != m_msaaBackBufferFbo)
        {
            ...
            uint32_t width  = m_resolution.m_width;
            uint32_t height = m_resolution.m_height;
            GLenum filter = BX_ENABLED(BGFX_CONFIG_RENDERER_OPENGL) ||
                            BX_ENABLED(BGFX_CONFIG_RENDERER_OPENGLES < 30)
                            ? GL_NEAREST
                            : GL_LINEAR
                            ;
    ...
    void ovrPostReset()
    {
        ...
        {
            const uint32_t msaaSamples = 1 << ( (m_resolution.m_flags & BGFX_RESET_MSAA_MASK) >>
    ...
    void updateCapture()
    {
        if (m_resolution.m_flags & BGFX_RESET_CAPTURE)
        {
            ...
            g_callback->captureBegin(m_resolution.m_width, m_resolution.m_height, m_resolution.m_width * 4,
    ...
    void capture()
    {
        if (NULL != m_capture)
        {
            ...
            if (GL_RGBA == m_readPixelsFmt)
            {
                bimg::imageSwizzleBgra8(m_capture, m_resolution.m_width, m_resolution.m_height,
                                        m_resolution.m_width * 4, m_capture);
            }
...
//                  break;
            ...
            default:
                BX_TRACE("%4d: INVALID 0x%08x, t %d, l %d, n %d, c %d", _uniformBuffer.getPos(), opcode, type, loc,
                         num, copy);
                break;
            }
                   ...
                   const float _palette[][4])
    {
        ...
        if (1 == numMrt)
        {
            ...
            if (0 != flags)
            {
                ...
                GL_CHECK(glScissor(_rect.m_x, _height - _rect.m_height - _rect.m_y, _rect.m_width,
                                   _rect.m_height) );
        ...
        else
        {
...

            {
                ...
                BX_CHECK(vertexDecl.m_stride == sizeof(Vertex),
                         "Stride/Vertex mismatch (stride %d, sizeof(Vertex) %d)", vertexDecl.m_stride, sizeof(Vertex) );
...
bool VRImplOVRGL::createSwapChain(const VRDesc& _desc, int _msaaSamples, int _mirrorWidth,
                                  int _mirrorHeight)
{
    if (!m_session)
    {
    ...
    if (NULL == m_textureSwapChain)
    {
        ...
        if (_msaaSamples > 1)
        {
            GL_CHECK(glRenderbufferStorageMultisample(GL_RENDERBUFFER, _msaaSamples, GL_DEPTH_COMPONENT32F,
                                                      width, height));
        }
        else
        {
        ...
        if (1 < _msaaSamples)
        {
            ...
            GL_CHECK(glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, _msaaSamples, GL_RGBA, width, height,
                                             GL_TRUE) );
            setDefaultSamplerState();

            GL_CHECK(glGenFramebuffers(1, &m_msaaTarget) );
            GL_CHECK(glBindFramebuffer(GL_FRAMEBUFFER, m_msaaTarget) );
            GL_CHECK(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE,
                                            m_msaaTexture, 0) );
...
bool VRImplOVRGL::submitSwapChain(const VRDesc& _desc)
{
    ...
    if (result != ovrSuccess_NotVisible && NULL != m_mirrorTexture)
    {
        ...
        GL_CHECK(glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0) );
        GL_CHECK(glBlitFramebuffer(0, m_mirrorHeight, m_mirrorWidth, 0, 0, 0, m_mirrorWidth, m_mirrorHeight,
    ...
    for (uint32_t ii = 0, iiEnd = m_usedCount; ii < iiEnd; ++ii)
    {
        ...
        if (-1 != loc)
        {
            if (UINT16_MAX != _vertexDecl.m_attributes[attr])
            {
                ...
                if ( (BX_ENABLED(BGFX_CONFIG_RENDERER_OPENGL >= 30) ||
...
)
{
    if (_target == GL_TEXTURE_3D)
    {
        GL_CHECK(glCompressedTexImage3D(
             ...
             ||  _target == GL_TEXTURE_CUBE_MAP_ARRAY)
    {
        compressedTexSubImage(_target, _level, 0, 0, _depth, _width, _height, 1, _internalformat,
                              _imageSize, _data);
    }
    else if (_target == GL_TEXTURE_2D_MULTISAMPLE_ARRAY)
    {
...
bool TextureGL::init(GLenum _target, uint32_t _width, uint32_t _height, uint32_t _depth,
                     uint8_t _numMips, uint32_t _flags)
{
    m_target  = _target;
    m_numMips = _numMips;
    m_flags   = _flags;
...
void TextureGL::create(const Memory* _mem, uint32_t _flags, uint8_t _skip)
{
    ...
    if (bimg::imageParse(imageContainer, _mem->data, _mem->size) )
    {
        ...
        {
            const bimg::ImageBlockInfo& ibi = bimg::getBlockInfo(bimg::TextureFormat::Enum(
...
void TextureGL::update(uint8_t _side, uint8_t _mip, const Rect& _rect, uint16_t _z, uint16_t _depth,
                       uint16_t _pitch, const Memory* _mem)
{
    ...
    const bool swizzle = true
                         ...
                         && !s_renderGL->m_textureSwizzleSupport
                         ;
    const bool unpackRowLength = BX_IGNORE_C4127(!!BGFX_CONFIG_RENDERER_OPENGL ||
    ...
    else
    {
        ...
        if (convert)
        {
            bimg::imageDecodeToRgba8(temp, data, width, height, srcpitch,
...
void TextureGL::setSamplerState(uint32_t _flags, const float _rgba[4])
{
    ...
    const uint32_t flags = (0 != (BGFX_TEXTURE_INTERNAL_DEFAULT_SAMPLER & _flags) ? m_flags : _flags) &
    ...
    if (hash != m_currentSamplerHash)
    {
        ...
        GL_CHECK(glTexParameteri(target, GL_TEXTURE_WRAP_S,
                                 s_textureAddress[(flags & BGFX_TEXTURE_U_MASK) >> BGFX_TEXTURE_U_SHIFT]) );
        GL_CHECK(glTexParameteri(target, GL_TEXTURE_WRAP_T,
                                 s_textureAddress[(flags & BGFX_TEXTURE_V_MASK) >> BGFX_TEXTURE_V_SHIFT]) );
        ...
        {
            GL_CHECK(glTexParameteri(target, GL_TEXTURE_WRAP_R,
                                     s_textureAddress[(flags & BGFX_TEXTURE_W_MASK) >> BGFX_TEXTURE_W_SHIFT]) );
        }
    ...
    if (0 != m_id)
    {
        if (GL_COMPUTE_SHADER != m_type)
        {
                ...
                &&  BX_ENABLED(BGFX_CONFIG_RENDERER_OPENGLES < 30) )
            {
                ...
                if (usesFragDepth)
                {
                    BX_WARN(s_extension[Extension::EXT_frag_depth].m_supported,
                            "EXT_frag_depth is used but not supported by GLES2 driver.");
                ...
                for (const char* fragDepth = bx::findIdentifierMatch(temp, "gl_FragDepth"); NULL != fragDepth;
...
void RendererContextGL::submit(Frame* _render, ClearQuad& _clearQuad,
                               TextVideoMemBlitter& _textVideoMemBlitter)
{
    ...
    const bool computeSupported = (BX_ENABLED(BGFX_CONFIG_RENDERER_OPENGL) &&
                                   s_extension[Extension::ARB_compute_shader].m_supported)
                                  || BX_ENABLED(BGFX_CONFIG_RENDERER_OPENGLES >= 31)
                                  ;
    ...
    if (0 == (_render->m_debug & BGFX_DEBUG_IFH) )
    {
        ...
        for (int32_t item = 0, restartItem = numItems; item < numItems || restartItem < numItems;)
        {
            ...
            if (isValid(draw.m_occlusionQuery)
                &&  !hasOcclusionQuery
                &&  !isVisible(_render, draw.m_occlusionQuery,
                               0 != (draw.m_submitFlags & BGFX_SUBMIT_INTERNAL_OCCLUSION_VISIBLE) ) )
            {
                continue;
            }
...
//                      }
                    ...
                    for (uint8_t ii = 0, num = frontAndBack + 1; ii < num; ++ii)
                    {
                        ...
                        if ( (BGFX_STENCIL_OP_FAIL_S_MASK | BGFX_STENCIL_OP_FAIL_Z_MASK | BGFX_STENCIL_OP_PASS_Z_MASK) &
                             changed)
                        {
                 ...
                 ) & changedFlags)
            {
                ...
                if (BX_ENABLED(BGFX_CONFIG_RENDERER_OPENGL) )
                {
                    ...
                    {
                        float pointSize = (float)(bx::uint32_max(1,
                                                                 (newFlags & BGFX_STATE_POINT_SIZE_MASK) >> BGFX_STATE_POINT_SIZE_SHIFT) );
                     ...
                     ||  blendFactor != draw.m_rgba)
                {
                         ...
                         ||  blendFactor != draw.m_rgba)
                    {
                        ...
                        const uint32_t equ    = uint32_t( (newFlags & BGFX_STATE_BLEND_EQUATION_MASK) >>
    ...
    if (_render->m_debug & (BGFX_DEBUG_IFH | BGFX_DEBUG_STATS) )
    {
        ...
        if (now >= next)
        {
            ...
            tvm.printf(10, pos++, 0x8e,
                       "    Submitted: %5d (draw %5d, compute %4d) / CPU %7.4f [ms] %c GPU %7.4f [ms] (latency %d) "
                       , _render->m_num
                       , statsKeyType[0]
                       , statsKeyType[1]
                       , elapsedCpuMs
            ...
            tvm.printf(10, pos++, 0x8e, " Uniform size: %7d, Max: %7d ", _render->m_uniformEnd,
            ...
            tvm.printf(10, pos++, attr[attrIndex & 1], " Submit wait: %7.4f [ms] ",
                       double(_render->m_waitSubmit)*toMs);
            tvm.printf(10, pos++, attr[(attrIndex + 1) & 1], " Render wait: %7.4f [ms] ",
                       double(_render->m_waitRender)*toMs);
